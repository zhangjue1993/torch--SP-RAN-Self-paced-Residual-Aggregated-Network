...
class WCE_sp(nn.Module):
    def __init__(self):
        super(WCE_sp, self).__init__()
    ...
    def forward(self, pred, targets):
        ...
        labels.size: [8, 256, 256]
        # '''
        B,H,W = pred[:,1,:,:].shape[0],pred[:,1,:,:].shape[1],pred[:,1,:,:].shape[2]
        loss = -torch.mean((targets * torch.log(pred[:,1,:,:]+1e-5)+(1-targets)*torch.log(pred[:,0,:,:]+1e-5)).view(B,-1), dim=-1)
        return loss
